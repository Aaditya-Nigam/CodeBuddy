...
export const CloneEditor = ({ fileId,projectId,cloneId }) => {
  const {getFile, saveCloneFile, isSaving, createCloneFile, getClone, handleSync, isSyncing} = useFileStore();
    ...
    <>
      <div className="flex items-center justify-between px-4 py-1 border-b-1 border-zinc-700">
        ...
        <div className="flex gap-2">
            <button to={`/push/${projectId}/${fileId}/${cloneId}`} className={`text-white cursor-pointer" title="Sync to main ${isSyncing?"animate-spin":""}`} onClick={handleSyncCode}><FaSync/></button>
