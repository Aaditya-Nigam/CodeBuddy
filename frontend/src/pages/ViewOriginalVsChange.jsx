...
export const ViewOriginalVsChange=()=>{
    ...
    const {getRequest,mergePullRequest,mergeConflict}=usePushPullStore()
    ...
    const handleConfirm=async ()=>{
        const res=await mergePullRequest(requestId)
        if(!res.hasConflict){
            toast.success("Merged Successfully!")
            navigate(-1)
        }else{
            toast.error("Conflict!")
            setConflict(res.mergedText)
            setIsConflict(true)
        }
    }
        ...
        <>
            <main className="bg-[#0d1117] min-h-[91.7vh] flex flex-col gap-6 p-4">
                ...
                <div className="flex justify-center gap-3">
                    <button className="bg-green-700 w-[30%] py-1 text-xl text-white rounded-lg cursor-pointer hover:bg-green-800" onClick={handleConfirm} disabled={isConflict}>Confirm</button>
                ...
                </div>
                {
                    ...
                    <>
                        ...
                        <div className="flex justify-center">
                            <button className="bg-green-700 w-[30%] py-1 text-xl text-white rounded-lg cursor-pointer hover:bg-green-800" onClick={handleConflictConfirm} disabled={!isConflict}>Confirm</button>
                        </div>
                    </>:
                    <></>
                }
